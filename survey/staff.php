...
$id = null;
$layout = PrivatePage('staff', '{{ST:staff_title}}');
...
$patients = $db->get_results("SELECT * FROM " . TABLES_PREFIX . "admin WHERE role = 4 ORDER BY id DESC LIMIT $offset, $rows");

$number_of_records = count($db->get_results("SELECT * FROM " . TABLES_PREFIX . "admin WHERE role = 4" ));
...
if($patients){
	foreach($patients as $patient){
		...
		$row_layout->SetContentView('staff-rows');
