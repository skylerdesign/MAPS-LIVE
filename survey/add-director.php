...
$layout = PrivatePage('add-director', '{{ST:administrators}}');
...
if(isset($_POST['submit'])){
	...
	$values['role'] = 1; // DIRECTOR
	
	if(isset($_POST['prefix']) AND $_POST['prefix'] != ''){
		$layout->AddContentById('prefix', $_POST['prefix']);
		$values['prefix'] = Clean($_POST['prefix']);
		$format[] = "%s";
	}
	...
	if(!$errors){
		...
		if($db->insert(TABLES_PREFIX . "admin", $values, $format)){
			...
			$layout->AddContentById('alert_message', '{{ST:the_director_has_been_saved}}');
		}else{
			$layout->AddContentById('alert', $layout->GetContent('alert'));
