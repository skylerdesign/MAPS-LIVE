...
$id = null;
$layout = PrivatePage('directors', '{{ST:staff_title}}');
...
$patients = $db->get_results("SELECT * FROM " . TABLES_PREFIX . "admin WHERE role = 1 ORDER BY id DESC LIMIT $offset, $rows");

$number_of_records = count($db->get_results("SELECT * FROM " . TABLES_PREFIX . "admin WHERE role = 1" ));
...
if($patients){
	foreach($patients as $patient){
		...
		$row_layout->SetContentView('director-rows');
		...
		$clinics = $db->get_results("SELECT * FROM clinics WHERE id = " . $patient->clinic);
