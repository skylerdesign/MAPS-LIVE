...
$id = null;
$layout = PrivatePage('analysis-by-substance', '{{ST:all_surveys}}');
...
if(isset($_GET['substance'])){
	...
	$layout->AddContentById('substance', 'substance: ' . $substance);
}

// Populate substance picklist
$layout->AddContentById('dropdown-substance', getSubstancePicklist());


// Get All Baseline Surveys By substance
switch ($substance) {
    case 'male':
        $$choice_id = 1;
        break;
    case 'Female':
        $choice_id = 2;
        break;
    case 'Transsubstance (male to female)':
        $choice_id = 3;
        break;
    case 'Transsubstance (female to male)':
    	$choice_id = 4;
    	break;
...
$baseline_results_by_substance = $db->get_results("SELECT * FROM survey_answers WHERE survey_id = 2 AND question_id = 1 AND choice_id = " . $substance );

if ( $baseline_results_by_substance ) {
	// Now look up each value
	foreach ( $baseline_results_by_substance as $baseline_result ) {
		// Get Answers
