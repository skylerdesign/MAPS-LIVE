...
function SaveMeta($id, $meta){
	...
	if($meta['meta_value'] == null){
		...
		$search = $db->get_row("SELECT * FROM " . TABLES_PREFIX . "quizmeta WHERE quiz_id = " . $id . " AND meta_key = '" . $meta['meta_key'] . "' ORDER BY id DESC");
		if($search){
...
function CreateSurvey( $survey, $event, $patient, $results, $clinician, $tablet ){
    $db = new Db;
    
    // Get last result set
    //$results_id = $db->get_row("SELECT results_id FROM " . TABLES_PREFIX . "answers ORDER BY survey_id DESC, results_id desc LIMIT 0,1");
    
	// First create the survey
    
    $survey_values = array();
    $survey_format = array();
    $survey_values['survey_id'] = $survey;
    $survey_format[] = "%s";
    $survey_values['results_id'] = $results_id;
    $survey_format[] = "%s";
    
	$db->insert(TABLES_PREFIX . "answers", $survey_values, $survey_format);
	
	$junction_values = array();
    $junction_format = array();
    $junction_values['patient_id'] = $patient;
    $junction_format[] = "%s";
    $junction_values['event_id'] = $event;
    $junction_format[] = "%s";
    $junction_values['survey_id'] = $survey;
    $junction_format[] = "%s";
    $junction_values['tablet_id'] = $tablet;
    $junction_format[] = "%s";
    
    $db->insert(TABLES_PREFIX . "junction", $junction_values, $junction_format);
    $junction_id = $db->insert_id;
...
function getUserDOB($id){
	...
	$user = $db->get_row("SELECT * FROM " . TABLES_PREFIX . "admin WHERE id = '" . $id . "' ORDER BY id DESC LIMIT 0,1");
...
function getPatientPicklist() {
	...
	if($patients){
		...
		foreach($patients as $patient){
			if ($id == $patient->id) {
				$temp .= '<option value="' . $patient->id . '" selected>' . $patient->lastname . ', ' . $patient->firstname . '</option>';
			} else {
				$temp .= '<option value="' . $patient->id . '">' . $patient->lastname . ', ' . $patient->firstname . '</option>';
			}
...
function getSubstancePicklist() {
	...
	$substances = array("Alcohol", "Cocaine", "Amphetamine", "Marijuana", "Opiates (e.g. Oxycontin, Heroin, etc)", "Sedatives (e.g. Valium, Ativan, Ambien)", "Hallucinogens (e.g. LSD, Ecstasy, Mushrooms)", "Inhalants", "Steroids");
	
	$temp = '<option></option>';

		foreach($substances as $substance){
			$key = array_search($substance, $substances)+1;
			if ($id == $key) {
				$temp .= '<option value="' . $key . '" selected>' . $substance . '</option>';
			} else {
				$temp .= '<option value="' . $key . '">' . $substance . '</option>';
			}
		}
...
function getChartTypePicklist() {
	...
	$types = array("Bar", "Line");
		...
		foreach($types as $type){
			if ($id == $type) {
				$temp .= '<option value="' . $type . '" selected>' . $type . '</option>';
			} else {
...
function getGenderPicklist() {
	...
	$id = $_GET['gender'];
	$genders = array("Male", "Female", "Transgender (male to female)", "Transgender (female to male)");
	
	$temp = '<option></option>';

		foreach($genders as $gender){
			$key = array_search($gender, $genders)+1;
			if ($id == $key) {
				$temp .= '<option value="' . $key . '" selected>' . $gender . '</option>';
			} else {
				$temp .= '<option value="' . $key . '">' . $gender . '</option>';
			}
		}
...
function getEventsByUserId($id) {
	...
	if($events){
		foreach($events as $event){
			...
			$result = $db->get_row("SELECT * FROM " . TABLES_PREFIX . "results WHERE event_id = $event_id AND patient_id = $patient_id AND survey_id = 4 ORDER BY id DESC LIMIT 0,1");
			if($result){
...
function isNoShow($id) {
	...
	// Get all events by user id 	
	$events = $db->get_row("SELECT * FROM events WHERE id = " . $id . " ORDER BY id ASC");
	
	$noshow = $events->noshow;
	
	return $noshow;
}

/** Track logins */
require_once( 'inc/track-logins.php' );
