...
$layout = PrivatePage('clinician', '{{ST:administrators}}');
...
if(isset($_GET['id'])){
	...
	Leave('clinicians.php');
}

if($id == intval($_SESSION['surveyengine_admin_user_id'])){
	echo $tmpl_strings->Get('cant_edit_yourself') . ' <a href="clinicians.php">'.$tmpl_strings->Get('back').'</a>';
	exit();
}
